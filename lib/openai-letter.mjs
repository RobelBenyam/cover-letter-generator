...
/** Required letter closing — do not paraphrase. */
...
function validateLetterConstraints(letter) {
  ...
  const hasClosing =
    normalizedEnd.includes("Draven@atlasco.ai") &&
    normalizedEnd.includes("Sincerely");
  ...
  if (!hasClosing) {
    issues.push(
      "Letter must end with the required closing: Sincerely, Draven Blake, Raleigh, NC, phone, Draven@atlasco.ai (exact block)."
    );
  }
  return issues;
}
...
export async function generateCoverLetter(openai, opts) {
  ...
  const userParts = [
    ...
    "Do not start with 'I am excited to apply' or 'I am writing to express'. Prefer: 'I'm applying for [role] at [company]' or jump straight into why you fit with one proof.",
    ...
    "Company fit paragraph: cite at least two specific phrases or requirements from the job description or company research notes (quoted or paraphrased tightly)—not generic praise.",
    ...
    `End the letter with this exact closing block (copy verbatim, including line breaks):\n${LETTER_CLOSING_BLOCK}`,
    ...
    messages: [
      {
        role: "system",
        content:
          'You write tailored job cover letters that sound written by a real senior salesperson — confident, plain-spoken, specific — never like generic AI output. No "resonates", "aligns perfectly", "I am excited to apply", "demonstrates consistent ability", or brochure tone. Keep letters concise: 250-320 words preferred, never over 350. The letter must include exactly one sentence that starts with "What makes these results repeatable is" and explains the candidate\'s repeatable operating system. Ground company fit in the job description or research notes. End with the exact closing block in the instructions. Never fabricate facts. Output only the letter body (no markdown fences).',
      },
...
- End with the exact closing block shown above
    ...
    messages: [
      {
        role: "system",
        content:
          'You revise cover letters per user instructions. The result must sound human — direct, specific, uneven rhythm — never like generic LLM output. Ban: resonates, aligns perfectly, I am excited to apply, demonstrates consistent ability, fast-paced environments, thrilled, leverage (verb). Optimize for sales storytelling; ground company fit in the JD or research notes. 250-320 words preferred, never over 350. Exactly one sentence starts with "What makes these results repeatable is" (explain operating system). End with the exact closing block in the user message. Never invent facts. Output valid JSON only with keys letter and message.',
      },
