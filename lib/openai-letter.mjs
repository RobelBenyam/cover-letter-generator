...
export async function generateCoverLetter(openai, opts) {
  ...
  const personalizationBlock = hasCompanyHook
    ? [
        ...
        "- Include at least two specific ties to the employer drawn only from the research block or explicit job-description lines (what they sell, who they serve, market, culture, tech focus, hiring story)—not generic praise.",
    ...
    messages: [
      {
        role: "system",
        content:
          "You write tailored job cover letters. Never fabricate candidate facts or employer facts beyond the provided job text and research block. Output only the letter body (no markdown fences).",
      },
    ...
    messages: [
      {
        role: "system",
        content:
          "You revise cover letters per user instructions. Never invent candidate facts or employer facts beyond the profile, job description, and research block. Output valid JSON only.",
      },
